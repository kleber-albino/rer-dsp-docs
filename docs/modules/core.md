...
Rodar sem argumentos (`./config.sh`). Antes do wizard, o script confere os repositórios irmãos (`rer-dsp-backend`, `rer-dsp-frontend`, `rer-dsp-job-data-migration`, `rer-dsp-job-geo-file-generation`) e oferece clonar o que faltar. Se já existir `config/adopter/adopter-config.yaml` de uma execução anterior, o script pergunta o que fazer antes de continuar:
...
3. Sobe os bancos (sem migração). Se `DSP_MIGRATION_EXECUTION_MODE=continuous`, mantém o job de migração ativo. No adotante real, sobe também `dsp-object-storage` e o job geo-file (`profile=object-storage`). A Demo Brasil não sobe object storage.
